...
var dicionario
const setDic = async () => { await fetch('cinco.txt').then(response => response.text()).then(text => {
...
const main = async () => { //Executa o programa depois do dicionário ter carregado
    ...
    for (let optiontwo = 0; optiontwo < palavra.length; optiontwo++) { //Cria os inputs onde será digitado as letras
...
botao.onclick = () => {
    ...
    for (let optiontwo = 0; optiontwo < dicionario.length; optiontwo++) { //Verifica se a palavra existe
    ...
    if (!conclusao) { //Adiciona a classificação "Não classificada" para a palavra caso não exista.
